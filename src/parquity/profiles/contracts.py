...
def admit_writer_profile_plan(
    ...
    if declared is None:
        return None
    writer_versions = tuple(writer.identity for writer in writers)
...
def _verify_metadata(
    ...
    if observation is ArtifactContractObservation.NOT_OBSERVABLE_EMPTY:
        return observation
    contract = _UNIVERSAL_PROFILE_CONTRACTS.get(profile.name)
    if contract is None:
        raise WriterProfileContractViolation(profile.name, "artifact contract is not recognized")
    contract(metadata, profile.name, expected_rows)
...
def _verify_gzip_contract(metadata: _FileMetadata, profile_name: str, expected_rows: int) -> None:
    del expected_rows
    _verify_compression(metadata, profile_name, "GZIP")


def _verify_brotli_contract(metadata: _FileMetadata, profile_name: str, expected_rows: int) -> None:
    del expected_rows
    _verify_compression(metadata, profile_name, "BROTLI")


def _verify_row_group_contract(
    metadata: _FileMetadata, profile_name: str, expected_rows: int
) -> None:
    _verify_row_groups(metadata, profile_name, expected_rows)
...
def _verify_statistics_contract(
    ...
    del expected_rows
    _verify_min_max_statistics(metadata, profile_name)


_UNIVERSAL_PROFILE_CONTRACTS: dict[str, Callable[[_FileMetadata, str, int], None]] = {
    "compression-gzip": _verify_gzip_contract,
    "compression-brotli": _verify_brotli_contract,
    "row-group-2": _verify_row_group_contract,
    "min-max-statistics-off": _verify_statistics_contract,
}
